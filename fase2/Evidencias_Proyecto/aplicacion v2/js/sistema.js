...
renderizarBomberos() {
    ...
    <button class="btn-accion-h btn-informe" onclick="generarInformeDeudas(${bombero.id})">📄 Informe Deudas</button>
...
verCuotas(id) {
    console.log('🎯 verCuotas llamado con ID:', id, 'tipo:', typeof id);
    Utils.mostrarNotificacion('Redirigiendo a cuotas y beneficios...', 'info');
    localStorage.setItem('bomberoCuotasActual', id);
    console.log('💾 Guardado en localStorage:', localStorage.getItem('bomberoCuotasActual'));
    ...
    try {
        ...
        // HEADER CON LOGO
        const tieneLogo = Utils.agregarLogoAPDF(doc, 5);
        const textStartX = tieneLogo ? 40 : 105;
        const textAlign = tieneLogo ? 'left' : 'center';
        
        doc.setFontSize(16);
        doc.setTextColor(211, 47, 47);
        doc.text('REPORTE DE DEUDORES', textStartX, 15, { align: textAlign });
        
        doc.setFontSize(10);
        doc.setTextColor(100);
        doc.text(`Fecha del reporte: ${new Date().toLocaleDateString('es-CL')}`, textStartX, 22, { align: textAlign });
...
async generarPDFConsultaVoluntarios() {
    ...
    try {
        ...
        const agregarHeader = (pageNum) => {
            ...
            const tieneLogo = Utils.agregarLogoAPDF(doc, 5);
            const textStartX = tieneLogo ? 38 : margin;
        ...
        bomberosOrdenados.forEach((bombero, index) => {
            ...
            doc.text(String(index + 1), 13, yPos, { align: 'center' });
