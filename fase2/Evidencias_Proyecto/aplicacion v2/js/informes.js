...
class GeneradorInformes {
    ...
    try {
        ...
        doc.text('INFORME DE DEUDAS', textStartX, 15, { align: textAlign });
        doc.setFontSize(12);
        doc.setFont(undefined, 'normal');
        doc.text('Compañía de Bomberos', textStartX, 25, { align: textAlign });
        doc.text(`Fecha: ${new Date().toLocaleDateString('es-CL')}`, textStartX, 32, { align: textAlign });
        yPos = 50;
        ...
        doc.setFontSize(11);
        doc.setFont(undefined, 'normal');
        doc.text(`Nombre: ${bombero.nombre}`, margin, yPos);
        yPos += 6;
        doc.text(`Clave: ${bombero.claveBombero}`, margin, yPos);
        yPos += 6;
        doc.text(`RUN: ${bombero.rut}`, margin, yPos);
        yPos += 6;
