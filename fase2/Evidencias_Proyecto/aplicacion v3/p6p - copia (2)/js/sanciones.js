...
async manejarSubmitFormulario(event) {
    ...
    // ⚠️ ELIMINADA VALIDACIÓN OBSOLETA DE NOMBRE Y RUT DEL FORMULARIO

    // Chequeo de seguridad:
    if (!this.bomberoActual || !this.bomberoActual.rut || !Utils.obtenerNombreCompleto(this.bomberoActual)) {
         Utils.mostrarNotificacion('Error: Datos del bombero (RUT/Nombre) no cargados. Recargue la página.', 'error');
         return;
    }

    // AHORA SOLO VALIDAMOS LOS DATOS DE LA SANCIÓN
    const errores = this.validarDatosSancion(datos);
    if (errores.length > 0) {
    ...
    configurarInterfaz() {
        ...
        document.getElementById('formSancion').addEventListener('submit', (e) => {
...
async manejarSubmitFormulario(event) {
    ...
    // ⚠️ ELIMINAMOS LA VALIDACIÓN OBSOLETA: Se confía en que this.bomberoActual está cargado.
    ...
    // Guardamos el ID del bombero para la recarga segura (F5)
    const bomberoIdParaRecarga = this.bomberoActual.id; 

    try {
        await this.guardarSancion(datos);
        Utils.mostrarNotificacion('✅ Sanción registrada exitosamente', 'success');
        
        // 🚨 CORRECCIÓN CLAVE DE PERSISTENCIA: Volver a guardar el ID en localStorage
        // Esto asegura que la página no muestre "Bombero no encontrado" al presionar F5.
        localStorage.setItem('bomberoSancionActual', bomberoIdParaRecarga);
...
repararIDsDuplicados() {
    ...
    this.bomberos.forEach(bombero => {
        if (idsUsados.has(bombero.id)) {
            ...
            const nuevoId = ++maxId;
            console.log(`🔄 Cambiando ID ${bombero.id} → ${nuevoId} para: ${bombero.nombre}`);
            bombero.id = nuevoId;
            cambios++;
        }
    ...
    async generarPDF() {
        ...
        try {
            ...
            // Datos centrados
            const antiguedad = Utils.calcularAntiguedadDetallada(this.bomberoActual.fechaIngreso);
            doc.text(`Nombre: ${this.bomberoActual.nombre}`, pageWidth / 2, yPos, { align: 'center' });
            yPos += 6;
            ...
            yPos += 6;
            doc.text(`RUN: ${this.bomberoActual.run}`, pageWidth / 2, yPos, { align: 'center' });
            yPos += 6;
...
volverAlSistema() {
    // ESTO ESTÁ BIEN: La clave solo debe eliminarse al SALIR del módulo.
