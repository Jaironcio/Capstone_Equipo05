...
class SistemaSanciones {
   ...
   async manejarSubmitFormulario(event) {
    ...
    try {
        ...
        const idEditando = idEditandoInput ? idEditandoInput.value : null;
        const esEdicion = idEditando && idEditando !== '' && idEditando !== 'null' && idEditando !== 'undefined';
        
        console.log('🔍 Estado edición:', { idEditando, esEdicion });
        ...
        const bomberoData = {
            ...
            email: datos.email || '',
            foto: fotoBase64,
            esVoluntarioMartir: datos.esVoluntarioMartir === 'on' ? true : false,
            otrosCuerpos: datos.otrosCuerpos || '',
            companiaOpcional: datos.companiaOpcional || '',
            desde: datos.desde || '',
            hasta: datos.hasta || '',
            // ✅ INCLUIR DATOS DE PADRINOS
            nombrePadrino1: datos.nombrePadrino1 || '',
            rutPadrino1: datos.rutPadrino1 || '',
            nombrePadrino2: datos.nombrePadrino2 || '',
            rutPadrino2: datos.rutPadrino2 || '',
        ...
        if (esEdicion) {
            ...
            const index = this.bomberos.findIndex(b => b.id === parseInt(idEditando));
            if (index !== -1) {
                this.bomberos[index] = bomberoData;
                Utils.mostrarNotificacion('Bombero actualizado exitosamente', 'success');
                console.log('✅ Bombero actualizado:', bomberoData.nombre);
            } else {
                Utils.mostrarNotificacion('Error: Bombero no encontrado para editar', 'error');
                return;
            }
        } else {
            // CREAR nuevo bombero
            ...
            console.log('✅ Nuevo bombero creado:', bomberoData.nombre);
        }

        // ✅ CORREGIDO: Usar guardarDatos() que ahora guarda TODO (bomberos + contadores)
        this.guardarDatos();
        
        // ❌ ELIMINADO: storage.saveCounters() - YA NO SE NECESITA porque guardarDatos() lo hace
