    ...
    static puedeReintegrarse(bombero) {
        ...
        switch(estado) {
            ...
            case 'renunciado':
                return {
                    puede: true,
                    mensaje: 'Puede reintegrarse en cualquier momento'
                };
                
            case 'separado':
                const hoy = new Date();
                const fechaFin = new Date(bombero.fechaFinSeparacion);
                
                if (hoy >= fechaFin) {
                    return {
                        puede: true,
                        mensaje: 'Ha cumplido el periodo de separación'
                    };
                } else {
                    const diasRestantes = Math.ceil((fechaFin - hoy) / (1000*60*60*24));
                    return {
                        puede: false,
                        mensaje: `Debe esperar ${diasRestantes} días más (hasta ${fechaFin.toLocaleDateString('es-CL')})`
                    };
                }
            ...
            case 'expulsado':
                return {
                    puede: false,
                    mensaje: 'No puede reintegrarse. Expulsión permanente.'
                };
