...
class SistemaPagarBeneficioDjango {
    ...
    async manejarPagoNormal(e) {
        ...
        // Obtener cantidad de tarjetas con validación
        const tarjetasRaw = formData.get('tarjetasVendidas') || formData.get('cantidadTarjetas') || formData.get('cantidadTarjetasPago');
        const cantidad_tarjetas = parseInt(tarjetasRaw) || 0;
        ...
        if (!cantidad_tarjetas || cantidad_tarjetas <= 0) {
            Utils.mostrarNotificacion('Error: Debe ingresar una cantidad válida de tarjetas', 'error');
            return;
        }
